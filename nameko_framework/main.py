...
from nameko.constants import AMQP_URI_CONFIG_KEY, RPC_EXCHANGE_CONFIG_KEY
...
def main(args):
    ...
    if args.config:
        ...
        if RPC_EXCHANGE_CONFIG_KEY not in config:
             config.update({RPC_EXCHANGE_CONFIG_KEY: args.exchange})
    else:
        config = {
